...
function S3StreamLogger(options) {
  ...
  this.uuid_name_with_hostname = options.uuid_name_with_hostname || false;
  ...
  if (!options.name_format) {
    ...
    if (this.uuid_name_with_hostname) {
      this.name_format = `${hostname()}-${uuid.v4()}${_extension}`;
    } else {
...
S3StreamLogger.prototype._upload = function (forceNewFile, cb) {
  ...
  this._prepareBuffer(
    function (err, buffer) {
      ...
      var tagging = Object.keys(this.tags)
        .map(function (key) {
...
S3StreamLogger.prototype._write = function (chunk, encoding, cb) {
  ...
  if (
    new Date().getTime() - this.last_write.getTime() > this.upload_every ||
